...
const parseDateTimeRangeFromText = (text: string): { startTime: string; endTime: string } | null => {
  if (!text) return null;
  const trimmed = text.trim();

  // en format: M/D/YYYY HH:mm - HH:mm
  let match = trimmed.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})\s+(\d{2}:\d{2})\s*-\s*(\d{2}:\d{2})$/);
  if (match) {
    const [, month, day, year, start, end] = match;
    const base = new Date(Number(year), Number(month) - 1, Number(day));
    if (!isNaN(base.getTime())) {
      const startTime = combineDateAndTime(base, start);
      const endTime = combineDateAndTime(base, end);
      if (startTime && endTime) return { startTime, endTime };
    }
  }

  // de format: D.M.YYYY HH:mm - HH:mm
  match = trimmed.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})\s+(\d{2}:\d{2})\s*-\s*(\d{2}:\d{2})$/);
  if (match) {
...
export default function Poll() {
  ...
  useEffect(() => {
    if (poll && !editDialogOpen) {
      setEditForm({
        ...
        expiresAt: formatLocalDateTime(poll.expiresAt) || "",
      ...
      setEditingOptions(
        poll.options?.map((opt) => {
          const parsed = poll.type === 'schedule' && !opt.startTime && !opt.endTime
            ? parseDateTimeRangeFromText(opt.text)
            : null;
          return {
            id: opt.id,
            text: opt.text,
            startTime: opt.startTime ? String(opt.startTime) : parsed?.startTime,
            endTime: opt.endTime ? String(opt.endTime) : parsed?.endTime,
  ...
  const updatePollMutation = useMutation({
    ...
    onError: (error: any) => {
      let description = t('pollView.toasts.changesNotSaved');
      if (error?.message) {
        try {
          const errorData = JSON.parse(error.message.split(': ').slice(1).join(': '));
          if (errorData?.errorCode === 'NO_FUTURE_OPTIONS') {
            description = t('pollView.reactivateNeedsFutureDates');
            setEditDialogOpen(true);
          } else if (errorData?.errorCode === 'POLL_EXPIRES_AT_PAST') {
            description = t('pollView.reactivateNeedsFutureExpiry');
            setEditDialogOpen(true);
          }
        } catch {}
      }
      toast({ title: t('pollView.toasts.error'), description, variant: "destructive" });
    },
  });
  ...
  const handleSaveAllChanges = async () => {
    ...
    if (poll?.type === 'schedule') {
      const hasInvalidRange = editingOptions.some((option) => {
        ...
        const hasAnyTime = !!option.startTime || !!option.endTime;
        if (!hasAnyTime) return false;
        if (!option.startTime || !option.endTime) return true;
    ...
    try {
      const pollUpdates = {
        ...editForm,
        expiresAt: editForm.expiresAt ? localToISOString(editForm.expiresAt) : null,
      };
      await updatePollMutation.mutateAsync(pollUpdates);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    {!poll.isActive && (
                      <div className="w-full space-y-1">
                        ...
                        {isPollExpired && (
                          <p className="text-xs text-muted-foreground">{t('pollView.reactivateExpiredHint')}</p>
                        )}
                      </div>
                    )}
                  </div>
          ...
          {(!hasVotes || hasVotesWarningShown) && (
            <>
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="edit-expires-at">{t('pollView.expiresAt')}</Label>
                  <Input
                    id="edit-expires-at"
                    type="datetime-local"
                    ...
                    <div className="mb-4 p-4 rounded-lg border-2 border-primary bg-primary/10 dark:bg-primary/20 space-y-3" data-testid="new-option-form">
                      ...
                      {poll?.type === 'schedule' ? (
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <div>
                              ...
                              <TimePickerDropdown
                                ...
                                onChange={(time) => {
                                  const currentDate = extractDateFromISO(newOptionForm.startTime) || extractDateFromISO(newOptionForm.endTime);
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            <Label className="text-xs text-muted-foreground">{t('pollView.date')} ({t('common.optional')})</Label>
                            <DatePicker
                              date={newOptionForm.date ? (() => { const [y,m,d] = newOptionForm.date!.split('-').map(Number); return new Date(y,m-1,d); })() : null}
                              onDateChange={(date) => {
                                const dateStr = date
                                  ? `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`
                                  : undefined;
                                setNewOptionForm({ ...newOptionForm, date: dateStr });
                              }}
                              placeholder={t('pollView.selectDate')}
                              inline
                              data-testid="new-option-date"
                            />
                          </div>
                          {newOptionForm.date && (
                            <>
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  ...
                                  <TimePickerDropdown
                                    ...
                                    onChange={(time) => {
                                      const endTime = newOptionForm.durationMinutes && time
                                        ? (() => {
                                            const [h, m] = time.split(':').map(Number);
                                            const endMin = h * 60 + m + newOptionForm.durationMinutes!;
                                            return `${String(Math.floor(endMin/60)).padStart(2,'0')}:${String(endMin%60).padStart(2,'0')}`;
                                          })()
                                        : newOptionForm.endTime;
                                      setNewOptionForm({ ...newOptionForm, startTime: time, endTime: endTime || '' });
                                    }}
                                    data-testid="new-option-start"
                                  />
                                </div>
                              ...
                              <div>
                                <Label className="text-xs text-muted-foreground">{t('pollView.duration')}</Label>
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {[15,30,45,60,90,120].map(d => (
                                    <button
                                      ...
                                      type="button"
                                      onClick={() => {
                                        const endTime = newOptionForm.startTime
                                          ? (() => {
                                              const [h, m] = newOptionForm.startTime.split(':').map(Number);
                                              const endMin = h * 60 + m + d;
                                              return `${String(Math.floor(endMin/60)).padStart(2,'0')}:${String(endMin%60).padStart(2,'0')}`;
                                            })()
                                          : newOptionForm.endTime;
                                        setNewOptionForm({ ...newOptionForm, durationMinutes: d, endTime: endTime || '' });
                                      }}
                                      className={`px-2 py-1 rounded border text-xs font-medium transition-colors ${newOptionForm.durationMinutes === d ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:border-primary/50'}`}
                                    >
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            let newOption: typeof editingOptions[0];
                            if (poll?.type === 'schedule') {
                              newOption = { text: newOptionForm.text, startTime: newOptionForm.startTime, endTime: newOptionForm.endTime, isNew: true };
                            } else if (poll?.type === 'organization') {
                              const dateObj = newOptionForm.date
                                ? (() => { const [y,m,d] = newOptionForm.date!.split('-').map(Number); return new Date(y,m-1,d); })()
                                : null;
                              newOption = {
                                text: newOptionForm.text,
                                startTime: dateObj && newOptionForm.startTime ? combineDateAndTime(dateObj, newOptionForm.startTime) : undefined,
                                endTime: dateObj && newOptionForm.endTime ? combineDateAndTime(dateObj, newOptionForm.endTime) : undefined,
                                maxCapacity: newOptionForm.maxCapacity,
                                isNew: true,
                              };
                            } else {
                              newOption = { text: newOptionForm.text, isNew: true };
                            }
                            setEditingOptions(prev => [...prev, newOption]);
                            setShowNewOptionForm(false);
                            setNewOptionForm({ text: '', startTime: '', endTime: '', maxCapacity: undefined, date: undefined, durationMinutes: undefined });
                          }}
                          disabled={(() => {
                            if (poll?.type === 'schedule') return !newOptionForm.startTime || !newOptionForm.endTime || isNewScheduleOptionRangeInvalid;
                            if (poll?.type === 'organization') {
                              if (!newOptionForm.text.trim()) return true;
                              const hasAnyTime = !!(newOptionForm.date || newOptionForm.startTime || newOptionForm.endTime);
                              const hasAllTime = !!(newOptionForm.date && newOptionForm.startTime && newOptionForm.endTime);
                              if (hasAnyTime && !hasAllTime) return true;
                              if (newOptionForm.startTime && newOptionForm.endTime && newOptionForm.startTime >= newOptionForm.endTime) return true;
                              return false;
                            }
                            return !newOptionForm.text.trim();
                          })()}
                                      ...
                                      <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <div>
                                          ...
                                          <TimePickerDropdown
                                            ...
                                            onChange={(time) => {
                                              const updated = [...editingOptions];
                                              const currentDate = extractDateFromISO(option.startTime) || extractDateFromISO(option.endTime);
                                              const newEndTime = currentDate ? combineDateAndTime(currentDate, time) : '';
                                              const newText = isAutoGeneratedText(option.text) 
                    ...
                    setEditingOptions(
                      poll.options?.map((opt) => {
                        ...
                        return {
                          id: opt.id,
                          text: opt.text,
                          startTime: formatLocalDateTime(opt.startTime) || parsed?.startTime,
                          endTime: formatLocalDateTime(opt.endTime) || parsed?.endTime,
                          maxCapacity: opt.maxCapacity ?? undefined,
                        };
                      }) || []
                    );
