...
- **Branch-Strategie (MANDATORY — NIEMALS VERLETZEN)**:
  - **Aktiver Arbeitsbranch für den aktuellen Beta-Release ist `feature/guest-access-control`**. Alle Änderungen für diesen Release werden dort entwickelt, committed und gepusht.
  ...
  - Vor jedem Push immer prüfen: `git branch` → nur pushen wenn der aktive Branch `feature/guest-access-control` ist.
