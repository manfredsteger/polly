...
push:
	...
	@echo "$(GREEN)Pushed: $(IMAGE_NAME):$(IMAGE_TAG)$(NC)"

publish: build push
	@echo "$(GREEN)Image published to $(IMAGE_NAME):$(IMAGE_TAG)$(NC)"
...
release:
	@read -p "Enter version (e.g., 0.1.0-beta.1): " version && \
	docker build -t $(IMAGE_NAME):$$version . && \
	docker push $(IMAGE_NAME):$$version && \
	docker tag $(IMAGE_NAME):$$version $(IMAGE_NAME):latest && \
	docker push $(IMAGE_NAME):latest && \
	echo "$(GREEN)Released: $(IMAGE_NAME):$$version + $(IMAGE_NAME):latest$(NC)"
