...
complete:
        ...
        @echo ""
        @echo "$(YELLOW)Step 1/5: Stopping and removing everything...$(NC)"
        docker compose --profile clamav down -v --remove-orphans 2>/dev/null || docker compose down -v --remove-orphans
        docker rmi $(IMAGE_NAME):$(IMAGE_TAG) 2>/dev/null || true
        @echo ""
        @echo "$(YELLOW)Step 2/5: Clearing Docker build cache...$(NC)"
        docker builder prune -af 2>/dev/null || true
        @echo ""
        @echo "$(YELLOW)Step 3/5: Rebuilding image from scratch...$(NC)"
        docker compose build --no-cache --pull
        @echo ""
        @echo "$(YELLOW)Step 4/5: Starting all services with ClamAV...$(NC)"
        SEED_DEMO_DATA=true CLAMAV_HOST=clamav CLAMAV_PORT=3310 CLAMAV_ENABLED=true docker compose --profile clamav up -d
        @echo ""
        @echo "$(YELLOW)Step 5/5: Waiting for services to be ready...$(NC)"
        @sleep 5
        @echo ""
